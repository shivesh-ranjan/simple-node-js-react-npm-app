pipeline {
	...
	stage('DAST') {
		...
		script {
		    ...
		    sh 'docker run --name zap -v /var/lib/jenkins/workspace/simple-node-js/:/zap/wrk/ --network="host" zaproxy/zap-stable zap-full-scan.py -t http://localhost:3000 -r zap-scan-report.html'
